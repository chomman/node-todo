...
describe('User', () => {
    ...
    it('/PUT update user', async () => {
        ...
        const { password: savedPassword, updatedAt: savedLastModifiedDate, ...savedUser } = await service.save(
            testUserDTO,
        );
        savedUser.firstName = 'Updated Name';

        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        const { password: updatedPassword, updatedAt: updatedLastModifiedDate, ...updatedUser } = (
            await request(app.getHttpServer())
                .put('/admin/users')
                .send(savedUser)
                .expect(200)
    ...
    it('/GET user with a login name', async () => {
        ...
        const getUser: UserDTO = (
            await request(app.getHttpServer())
                .get('/admin/users/' + savedUser.login)
                .expect(200)
        ).body;
    ...
    it('/DELETE user', async () => {
        ...
        await request(app.getHttpServer())
            .delete('/admin/users/' + savedUser.login)
            .expect(204);
